...
altair_mailbox = {
                ...
                begin: function() {
                  $msg_list.each(function(i) {
                    0 != i &&
                      $(this).velocity(
                        { marginTop: 0 },
                        { duration: anim_duration * i + 200, easing: easing_swiftOut }
                      );
                  });
                },
                complete: function() {
                  $mailbox
                    ...
                    .velocity('transition.expandIn', { duration: anim_duration, easing: easing_swiftOut });
                }
              });
          },
          complete: function() {
            $split_view_btn.velocity('transition.expandIn', { duration: anim_duration, easing: easing_swiftOut });
          }
        });
    }),
        ...
        i.preventDefault(),
          $split_view_btn.velocity('transition.expandOut', {
            ...
            begin: function() {
              ...
              $mailbox
                .find('.md-card-list-header-combined')
                .velocity('transition.expandOut', { duration: anim_duration, easing: easing_swiftOut }),
                i.reverse().each(function(i) {
                  i != t - 1 && $(this).velocity('reverse', { duration: anim_duration + 200, easing: easing_swiftOut });
                }),
                setTimeout(function() {
            ...
            complete: function() {
              $combined_view_btn.velocity('transition.expandIn', { duration: anim_duration, easing: easing_swiftOut });
            }
          });
      }),
  ...
  list_category_color: function() {
    $('.md-card-list > ul > li').each(function() {
      ...
      i &&
        $(this)
          .find('span.md-card-list-item-avatar')
  ...
  toggle_message: function() {
    $mailbox.on('click', '.md-card-list ul > li', function(i) {
      if (
        !$(i.target).closest('.md-card-list-item-menu').length &&
        !$(i.target).closest('.md-card-list-item-select').length
      ) {
        var t = $(this);
        if (!t.hasClass('item-shown')) {
          ...
          $mailbox.find('.item-shown').velocity('reverse', {
            ...
            t.velocity(
              { marginTop: 40, marginBottom: 40, marginLeft: -20, marginRight: -20, minHeight: e },
              {
                duration: 300,
                easing: easing_swiftOut,
                begin: function(i) {
                ...
                complete: function(i) {
                  ...
                  var t = $('body'),
                    e = $(i);
                  t.animate({ scrollTop: e.offset().top - $page_content.offset().top - 8 }, 1e3, bez_easing_swiftOut);
                }
              }
            );
        }
      ...
      $(document).on('click keydown', function(i) {
        ($(i.target).closest('li.item-shown').length && 27 != i.which) ||
          $mailbox.find('.item-shown').velocity('reverse', {
            begin: function(i) {
              $(i)
                .removeClass('item-shown')
  ...
  create_message: function() {
    ...
    var i = $('#mail_progressbar'),
      ...
      e = {
        ...
        error: function(e) {
          i.addClass('uk-progress-danger'), t.css({ width: '100%' }).text('100%');
        },
        abort: function(i) {
          console.log(i);
        },
